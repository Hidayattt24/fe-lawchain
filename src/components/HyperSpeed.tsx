...
interface HyperspeedProps {
  effectOptions?: Partial<Omit<HyperspeedOptions, "colors">>;
  colors?: Colors;
  className?: string;
}
...
class App {
  ...
  updateColors(colors: Colors) {
    // Update fog color
    if (this.scene.fog && this.fogUniforms.fogColor) {
      (this.scene.fog as THREE.Fog).color.setHex(colors.background);
      (this.fogUniforms.fogColor.value as THREE.Color).setHex(
        colors.background
      );
    }
    ...
    if (
      this.road &&
      this.road.leftRoadWay &&
      this.road.rightRoadWay &&
      this.road.island
    ) {
      // Update left road
      const leftRoadMaterial = this.road.leftRoadWay
        .material as THREE.ShaderMaterial;
      if (leftRoadMaterial.uniforms.uColor) {
        (leftRoadMaterial.uniforms.uColor.value as THREE.Color).setHex(
          colors.roadColor
        );
      }
      if (leftRoadMaterial.uniforms.uBrokenLinesColor) {
        (
          leftRoadMaterial.uniforms.uBrokenLinesColor.value as THREE.Color
        ).setHex(colors.brokenLines);
      }
      if (leftRoadMaterial.uniforms.uShoulderLinesColor) {
        (
          leftRoadMaterial.uniforms.uShoulderLinesColor.value as THREE.Color
        ).setHex(colors.shoulderLines);
      }

      // Update right road
      const rightRoadMaterial = this.road.rightRoadWay
        .material as THREE.ShaderMaterial;
      if (rightRoadMaterial.uniforms.uColor) {
        (rightRoadMaterial.uniforms.uColor.value as THREE.Color).setHex(
          colors.roadColor
        );
      }
      if (rightRoadMaterial.uniforms.uBrokenLinesColor) {
        (
          rightRoadMaterial.uniforms.uBrokenLinesColor.value as THREE.Color
        ).setHex(colors.brokenLines);
      }
      if (rightRoadMaterial.uniforms.uShoulderLinesColor) {
        (
          rightRoadMaterial.uniforms.uShoulderLinesColor.value as THREE.Color
        ).setHex(colors.shoulderLines);
      }

      // Update island
      const islandMaterial = this.road.island.material as THREE.ShaderMaterial;
      if (islandMaterial.uniforms.uColor) {
        (islandMaterial.uniforms.uColor.value as THREE.Color).setHex(
          colors.islandColor
        );
      }
    }
...
const HyperSpeed: React.FC<HyperspeedProps> = ({
  ...
  useEffect(() => {
    const initHyperspeed = async () => {
      try {
        console.log("Initializing HyperSpeed...");
        ...
        const container = containerRef.current;
        if (!container) {
          console.error("Container not found for HyperSpeed");
          return;
        }

        console.log("Container found, creating HyperSpeed app...");
        ...
        console.log("HyperSpeed options:", options);
  ...
  }, [effectOptions]); // Only depend on effectOptions, not colors

  // Update colors without restarting the animation
  useEffect(() => {
    if (appRef.current && colors) {
      console.log("Updating HyperSpeed colors...");
      appRef.current.updateColors(colors);
    }
  }, [colors]);
